...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        for _ in range(n_layers):
            # output_placeholder = (DONE) TODO # HINT: use tf.layers.dense (specify <input>, <size>, activation=<?>)
            output_placeholder = tf.layers.dense(input=output_placeholder, size=output_size, activation=output_activation)
        output_placeholder = tf.layers.dense(input=output_placeholder, size=output_size, activation=output_activation)
