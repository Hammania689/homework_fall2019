...
class ReplayBuffer(object):
    ...
    def sample_random_data(self, batch_size):
        ...
        ## (DONE) TODO return batch_size number of random entries from each of the 5 component arrays above
        ## HINT 1: use np.random.permutation to sample random indices
        ## HINT 2: return corresponding data points from each array (i.e., not different indices from each array)
        ## HINT 3: look at the sample_recent_data function below
        # NOTE use np.random.randint instead of permutation. This should accomplish the task in a much simpler fashion
        random_indices = np.random.randint(0, high=self.obs.shape[0], size=batch_size)
